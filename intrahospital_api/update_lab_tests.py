...
def clean_queryset(queryset):
    ...
    for row in queryset:
        ...
        for lab_test in row["lab_tests"]:
            ...
            if not lab_test["external_identifer"]:
                continue
...
def update_lab_tests_from_query(queryset):
    ...
    existing_hns = [
        i['demographics']["hospital_number"] for i in queryset
    ]

    hospital_number_to_patient_id = zip(elcid_models.Demographics.objects.filter(
        hospital_number__in=existing_hns
    ).values_list('hospital_number', 'patient_id', flat=True))

    queryset = remove_non_elcid_patients(queryset, set(hospital_number_to_patient_id.keys()))
    ...
    for row in queryset:
        ...
        for lab_test_dict in row["lab_tests"]:
            lab_tests_to_create.append(
                cast_to_lab_test(patient_id, lab_test_dict)
            )
    ...
    for row in queryset:
        ...
        patient_id = hospital_number_to_patient_id[hospital_number]
        for lab_test_dict in row["lab_tests"]:
            lab_number = lab_test_dict["external_identifer"]
            test_name = lab_test_dict["test_name"]
            lab_test = patient_id_lab_number_test_name[(patient_id, lab_number, test_name,)]
