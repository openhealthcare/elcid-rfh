...
class LoadIPCTestCase(OpalTestCase):
	...
	def test_creates_if_there_is_a_bed_status(self, ProdAPI):
		ProdAPI.return_value.execute_hospital_query.return_value = [self.upstream_row]
		self.patient.bedstatus.create(
			bed_status='Occupied'
		)
		self.cmd.handle()
		self.assertTrue(
	...
	def test_ignores_if_there_is_no_bed_status(self, ProdAPI):
		ProdAPI.return_value.execute_hospital_query.return_value = [self.upstream_row]
		self.cmd.handle()
		self.assertFalse(
			self.patient.episode_set.filter(
		...
		ipc_status = self.patient.ipcstatus_set.get()
		# Make sure the patient's ipc status has not been updated by the row
		self.assertIsNone(ipc_status.comments)
